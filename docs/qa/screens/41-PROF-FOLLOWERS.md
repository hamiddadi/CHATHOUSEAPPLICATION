...
- **Pre-conditions globales** : compte authentifie ; reseau pour charger les listes (`GET /follow/:userId/followers` et `GET /follow/:userId/following`, envelope paginee `{ data, nextCursor, hasMore }`). `onEndReached` charge toutes les pages avec le curseur opaque `(createdAt,id)` ; un curseur invalide retourne `VALIDATION_001`/400. Reseau egalement requis pour muter le follow.
